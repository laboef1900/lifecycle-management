...
### Window divergence (write path vs. read path) — fails safe only under a past-dated anchor
...
This divergence is **left in place on purpose because it fails safe _when the newest baseline's `capturedAt` is not later than today_** (the normal case). The reviewer's flagged 422 is the price:

- **Scoped premise (past-dated anchor only):** when `capturedAt ≤ today`, the baseline-anchored write window starts **no later than** any chip window, so the snapshotted `orderByDate` is **never later** than the live one. The `≥ T` worsening rule (INV-5) supersedes only when the live `orderByDate` is _earlier_ than the approved one — so with a write window that starts earliest, a genuine worsening reads as "improving/unchanged" and the approval can only **linger**, never **falsely vanish**, on _any_ view. Live chip urgency (`orderByDate` vs `today`) still escalates independently, so a lingering acknowledgment can never hide a now-urgent order. **This never-falsely-supersede property holds ONLY under `capturedAt ≤ today`; it is not an unconditional invariant** (see the future-dated limitation below).
- **Known limitation — future-dated baseline (behavioral fix tracked as [#303]):** future-dated baselines are accepted elsewhere **with no upper bound** (see the `forecast-loader.ts` anchor `@ai-warning`: `capturedAt <= today` is deliberately _not_ enforced). When the newest baseline's `capturedAt` is **later than today**, the baseline-anchored write window starts **later** than the today-anchored chip window, so the snapshotted `orderByDate` can be **later** than the live one — and the `≥ T` rule then **falsely supersedes the approval the instant it is created**, so the acknowledgment never appears. Reproduced by an adversarial re-review with a baseline dated ~9 months out. This is a genuine behavioral defect, **not** a confusing-but-safe symptom, and the scoped premise above does **not** cover it. The behavioral fix (e.g. anchoring the write window at `min(baseline, today)`, or matching the chip window) is tracked separately as **#303**; this PR documents the limitation only and does not change the coverage/window logic.
- **Rejected alternative — align the write window to `today`:** it would remove the 422 on the default 24-mo view, but the **`all` view** (whose `from` is the baseline, i.e. earlier than `today`) would then compute a live `orderByDate` _earlier_ than a today-anchored snapshot, tripping the `≥ T` rule and making the acknowledgment **disappear when the operator merely switches to the `all` window** — a worse, view-dependent regression on the purchasing surface. Anchoring the write window earliest is the more robust choice **for past-dated anchors** (but see the future-dated limitation above, which a full fix under #303 must also handle).
- **One visible-but-safe symptom (past-dated anchor):** a `422 NO_LIVE_BREACH` on Approve for a breach that sits past the write window's `to` (`anchor + horizon`) yet within the chip's `to` (`today + horizon`) — only reachable when the anchor is stale by more than one month _and_ the breach is >24 months out. The chip shows the breach; Approve reports "no live breach". Confusing, but it can never mis-record or mask an order.

The `liveBreachContext` doc comment states this accurately (it previously, wrongly, claimed "exactly what the chip shows"). A future change MAY align the windows by passing the displayed `from`/`to` into the approve request, but that widens the contract (client-controlled snapshot window) and must preserve the scoped never-falsely-supersede property above **and** fix the future-dated-baseline case (#303).
