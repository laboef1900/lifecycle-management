...
**4. Host→cluster membership is time-scoped.** Recorded 2026-07-22 (#289). A host contributes capacity to the cluster it belonged to **in that month**, not to whatever cluster it sits in today. Membership is an interval timeline (`host_cluster_memberships`, half-open `[effectiveFrom, effectiveTo)`, `effectiveTo IS NULL` = current); a move (`POST /api/hosts/:id/move`) closes the open interval at the move date and opens a new one, so the forecast credits the OLD cluster before the move and the NEW cluster after it. The rejected alternative was a naive `Host.clusterId` flip: because attribution had no member-since date, a flip re-attributed the host's **entire** capacity history across **all** modelled months, retroactively rewriting **both** clusters' forecasts on a purchasing surface. Capacity rows stay FK'd to `host_id` and are never moved — attribution is resolved through the timeline at forecast time (`forecast-loader.ts` loads hosts by membership `clusterId`; `forecast.ts` gates each month on the covering interval). Invariants (app-enforced, not by DB constraint — same reasoning as the serial-number index, #123): exactly one open membership per host; intervals contiguous and non-overlapping; `Host.clusterId` equals the open membership's cluster. Synced hosts are exempt from the manual move — `reconcileHosts` owns their `clusterId` and maintains their membership on every pass.
