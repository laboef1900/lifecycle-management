...
### Window divergence (write path vs. read path) — anchor clamped to `min(capturedAt, today)` ([#303])

The forecast is evaluated over a `[from, to]` window. The **write path** (`ForecastService.liveBreachContext`, invoked by `OrderApprovalService.create`) uses the **server default** window with its anchor **clamped to `min(capturedAt, today)`** (`clampAnchorToToday`, #303): `from = firstOfMonth(min(newest baseline capturedAt, today))`. The **read path** the chip renders uses whatever window the web client requests, which for the 12/24-mo views is **today-anchored** (`resolveWindow` in `window-controls.tsx`: `from = firstOfMonth(today)`). The clamp keeps the write path baseline-anchored for the normal case while removing the future-dated-baseline defect:

- **Past/present-dated anchor (`capturedAt ≤ today`, the normal case) — clamp is a no-op:** `min` returns `capturedAt`, so the write window is exactly the baseline-anchored one and starts **no later than** any chip window. The snapshotted `orderByDate` is **never later** than the live one. The `≥ T` worsening rule (INV-5) supersedes only when the live `orderByDate` is _earlier_ than the approved one — so with a write window that starts earliest, a genuine worsening reads as "improving/unchanged" and the approval can only **linger**, never **falsely vanish**, on _any_ view. Live chip urgency (`orderByDate` vs `today`) still escalates independently, so a lingering acknowledgment can never hide a now-urgent order.
- **Future-dated anchor (`capturedAt > today`) — clamp fixes the defect ([#303]):** future-dated baselines are accepted elsewhere **with no upper bound** (see the `forecast-loader.ts` anchor `@ai-warning`: `capturedAt <= today` is deliberately _not_ enforced). Before the clamp, the raw baseline-anchored write window started **later** than the today-anchored chip window, so the snapshotted `orderByDate` landed **later** than the live one and the `≥ T` rule **falsely superseded the approval the instant it was created** (the acknowledgment never appeared; reproduced by adversarial re-review with a baseline dated ~9 months out). Clamping the anchor to `today` pulls the write window start onto `firstOfMonth(today)`, matching the chip's 12/24-mo anchor, so the snapshotted `orderByDate` equals the live one and the fresh approval is acknowledged. The fix is at the **write path**, not the accept path, so future-dated baselines keep their (deliberate, unbounded) forecast semantics.
- **Rejected alternative — align the whole write window to the chip's `[from, to]`:** the clamp only moves the write **anchor**; it does **not** widen the contract to a client-controlled snapshot window, and it does **not** touch the read path. Fully aligning the write window to `today` (dropping the baseline anchor) was rejected because the **`all` view** (whose `from` is the baseline, i.e. earlier than `today`) would then compute a live `orderByDate` _earlier_ than a today-anchored snapshot, tripping the `≥ T` rule and making the acknowledgment **disappear when the operator merely switches to the `all` window** — a worse, view-dependent regression on the purchasing surface. Clamping to `min(capturedAt, today)` is the smallest-blast-radius fix: it corrects the future-dated case without reopening that `all`-view regression.
- **One visible-but-safe residual symptom (past-dated anchor):** a `422 NO_LIVE_BREACH` on Approve for a breach that sits past the write window's `to` (`anchor + horizon`) yet within the chip's `to` (`today + horizon`) — only reachable when the anchor is stale by more than one month _and_ the breach is >24 months out. Unchanged by #303 (the clamp is a no-op for a past anchor). The chip shows the breach; Approve reports "no live breach". Confusing, but it can never mis-record or mask an order.

The `liveBreachContext` doc comment states this accurately. A future change MAY align the windows further by passing the displayed `from`/`to` into the approve request, but that widens the contract (client-controlled snapshot window) and must preserve the never-falsely-supersede property above.

[#303]: https://github.com/laboef1900/lifecycle-management/issues/303
