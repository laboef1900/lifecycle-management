...
**This section covers the window itself and the read that honours it.** The sweep that actually deletes rows is a separate change (§12.1) and is deliberately not described here — the read bound has to exist first, which is what makes deleting safe.
...
- **INV-R1a — an out-of-spec window disables retention; it is never clamped.** Any value outside `{0} ∪ [12, 120]` yields `null` (retention off). The schema rejects those on write, so reaching the function means a direct DB write bypassed it — and on a destructive operation the safe answer is to prune nothing rather than to invent a window nobody configured. Clamping would convert tampering into real, unrecoverable deletes, which is precisely the weaker default the resilience rule forbids. The sweep logs the case at `warn` (§12.1) so it does not pass silently. Mirrors `SettingsService.coerceBandWidth`, which fails an out-of-enum band width safe to the default for the same reason.
