...
- **Origin:** [#272](https://github.com/laboef1900/lifecycle-management/issues/272) follow-up. Real-world failure: a default vCenter presents its Machine SSL **leaf without the VMCA root in the handshake**, so root-pinning has nothing to pin ("vCenter did not present its root CA, so there is no certificate to pin").
- **Supersedes:** **D11** of `docs/vsphere-integration-design.md` (pin the chain root as a `ca:` anchor). **Amends** the reasoning in **§0.1** (does _not_ re-introduce a rejected "ignore TLS" flag) and **honors D10** (the `checkServerIdentity` trap) in full.
- **Date:** 2026-07-21
...
The underlying need is unchanged from §0.1: **a self-signed / privately-issued vCenter must "just work," with verification effectively ON.** D11's mechanism is what fails, not the goal.
